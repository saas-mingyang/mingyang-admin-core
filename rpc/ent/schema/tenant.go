...
func (Tenant) Fields() []ent.Field {
	return []ent.Field{
		...
		field.String("contact_email").Optional().Comment("联系邮箱"),
		field.String("company_name").Optional().Comment("企业名称"),
		field.String("license_number").Optional().Comment("统一社会信用代码"),
		field.String("address").Optional().Comment("地址"),
		field.String("intro").Optional().Comment("企业简介"),
		field.String("domain").Optional().Comment("域名"),
		field.Uint32("level").Comment("租户级别"),
		field.Int64("plan_id").Optional().Comment("套餐计划Id，外键关联 tenant_plan.id"),
		field.Int64("admin_id").Optional().Positive().Comment("管理员id"),
		field.Int64("parent_id").Optional().Comment("父级id,0为第一级"),
	}
}
